...
public class PoblarArtistas {
    ...
    public static void main(String[] args) {
        try (MongoClient mongoClient = MongoClients.create("mongodb://localhost:27017")) {
            ...
            System.out.println("Generando 15 Solistas con 2 álbumes y 3 canciones cada uno...");
            artistasParaInsertar.addAll(generarSolistas());

            System.out.println("Generando 15 Bandas con integrantes, álbumes y canciones...");
            artistasParaInsertar.addAll(generarBandas());

            coleccionArtistas.insertMany(artistasParaInsertar);
            
            System.out.println("¡Éxito! Se insertaron " + artistasParaInsertar.size() + " artistas, junto con 60 álbumes y 180 canciones.");
    ...
    private static List<Document> generarBandas() {
        ...
        bandas.add(crearBanda("Metallica", "url_metallica.jpg", "Heavy Metal", Arrays.asList(
            ...
            crearIntegrante("Cliff Burton", "Bajista", "1982", "1986", "Inactivo")
        )));
    ...
    private static Document crearSolista(String nombre, String imagen, String genero) {
        return new Document("tipo", "Solista")
                ...
                .append("albumes", generarAlbumesDePrueba(nombre, genero));
    }
    ...
    private static List<Document> generarAlbumesDePrueba(String nombreArtista, String genero) {
        List<Document> cancionesAlbum1 = Arrays.asList(
                new Document("titulo", "Éxito 1 de " + nombreArtista).append("duracion", "3:15"),
                new Document("titulo", "Éxito 2 de " + nombreArtista).append("duracion", "4:02"),
                new Document("titulo", "Éxito 3 de " + nombreArtista).append("duracion", "2:50")
        );

        List<Document> cancionesAlbum2 = Arrays.asList(
                new Document("titulo", "Clásico 1 de " + nombreArtista).append("duracion", "3:40"),
                new Document("titulo", "Clásico 2 de " + nombreArtista).append("duracion", "3:10"),
                new Document("titulo", "Clásico 3 de " + nombreArtista).append("duracion", "4:20")
        );

        return Arrays.asList(
                new Document("nombre", "Primer Álbum de " + nombreArtista)
                        .append("fechaLanzamiento", "2015-05-10")
                        .append("genero", genero)
                        .append("imagenPortada", "portada1_" + nombreArtista.replace(" ", "") + ".jpg")
                        .append("canciones", cancionesAlbum1),

                new Document("nombre", "Segundo Álbum de " + nombreArtista)
                        .append("fechaLanzamiento", "2020-11-22")
                        .append("genero", genero)
                        .append("imagenPortada", "portada2_" + nombreArtista.replace(" ", "") + ".jpg")
                        .append("canciones", cancionesAlbum2)
        );
    }
}